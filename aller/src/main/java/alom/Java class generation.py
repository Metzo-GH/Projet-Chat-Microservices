...
def combine_java_files(current_dir, output_file):
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile:
        ...
        for root, dirs, files in os.walk(current_dir):  # On commence à partir du répertoire courant
            # Vérifier que le répertoire courant est un sous-répertoire du répertoire de départ
            if not root.startswith(current_dir):
                continue
            for file in files:
...
if __name__ == '__main__':
    ...
    combine_java_files(current_dir, output_file)
